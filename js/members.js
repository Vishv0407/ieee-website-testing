document.addEventListener("DOMContentLoaded", function () {
  ...
  function fetchData() {
    axios.get('http://localhost:3000/api/members/allMembers')
      .then(response => {
  ...
  window.editMember = function(id) {
    axios.post(`http://localhost:3000/api/members/update/${id}`)
      .then(response => {
  ...
  window.deleteMember = function (id) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        axios.delete(`http://localhost:3000/api/members/member/${id}`)
          .then(response => {
            fetchData();
            Swal.fire(
              'Deleted!',
  ...
  form.addEventListener('submit', function (event) {
    ...
    const url = memberId ? `http://localhost:3000/api/members/update/${memberId}` : 'http://localhost:3000/api/members/upload';
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  async function fetchMembers() {
    try {
      const response = await axios.get('http://localhost:3000/api/members/allMembers'); // Adjust the URL to your actual API endpoint
