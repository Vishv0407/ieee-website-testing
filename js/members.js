  ...
  window.deleteMember = function (id) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        axios.delete(`http://localhost:3000/api/member/${id}`)
          .then(response => {
            fetchData();
            Swal.fire(
              'Deleted!',
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  async function fetchMembers() {
    try {
      const response = await axios.get('http://localhost:3000/api/members'); // Adjust the URL to your actual API endpoint
