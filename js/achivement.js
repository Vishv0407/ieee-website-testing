document.addEventListener("DOMContentLoaded", function () {
    ...
    window.editAchievement = function (id) {
        axios.post(`http://localhost:3000/api/achievements/update/${id}`)
            .then(response => {
    ...
    window.deleteAchievement = function (id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                axios.delete(`http://localhost:3000/api/achievement/${id}`)
                    .then(response => {
                        fetchAchievements();
    ...
    form.addEventListener('submit', function (event) {
        ...
        if (achievementId) {
            axios.post(`http://localhost:3000/api/achievements/update/${achievementId}`, formData)
                .then(handleResponse)
                .catch(handleError);
        } else {
            axios.post('http://localhost:3000/api/achievements/upload', formData)
                .then(handleResponse)
                .catch(handleError);
        }
    });
